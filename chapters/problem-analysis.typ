...
The second method of data collection involves equipping the vehicle with a large number of additional sensors and aggregating data from the vehicle's internal systems using dedicated supporting hardware (similar to @img:etas-hardware). In practice, this means lifting the vehicle, disassembling parts of the engine bay, and installing various sensors to measure parameters not available through the OBD-II port. This can include additional temperature, pressure, and accelerometer sensors, as well as other specialized equipment, depending on the specific requirements of the test. An example of such a setup is shown in Attachment 2, where various sensors are installed in the engine bay of a test vehicle. The data from these sensors, along with signals from the car's internal systems, is then aggregated using dedicated hardware, such as the ETAS data acquisition system (as shown in Attachments 4 and 5), which can handle high sampling rates and large volumes of data.
...
For example, the #abbr("ECU") often regulates certain components through feedback control loops rather than providing direct measurements. A typical case is the main radiator fan, whose operation is controlled via #abbr("PWM", "Pulse-Width Modulation") to adjust cooling power dynamically. As a result, the fan speed does not directly correspond to the engine speed, and the actual fan rotational speed is not always available as a measured signal. In such cases, the fan speed may need to be estimated indirectly from related variables such as voltage, current, and #abbr("PWM") duty cycle. By installing an additional sensor that directly measures the fan's rotational speed, the system can provide a precise, readily usable measurement, significantly simplifying subsequent analysis.

In this configuration, the logging device is typically a laptop computer running specialized third-party software designed for high-fidelity data acquisition (as illustrated in Attachment 3). At Škoda Auto, the software commonly used for this purpose is #abbr("INCA", "Integrated Calibration and Acquisition System"), a commercial tool developed by ETAS GmbH. This software provides advanced capabilities for data acquisition, calibration, and analysis, making it well-suited for the complex requirements of high-resolution telemetry data collection @etas_inca_products.

Compared to the OBD-II-based method, this approach generates significantly larger datasets, often containing measurements from thousands of sensors at different high sampling rates. A simple text-based format would be impractical for storing such large volumes of data, so a more sophisticated, standardized format is used. The most common format for this type of data is the #abbr("MDF", "Measurement Data Format"), a binary file format designed to store large volumes of measurement data efficiently. The MDF format allows for the storage of complex datasets with multiple channels, varying sampling rates, and metadata, making it ideal for high-fidelity telemetry data @asam_mdf. The majority of technicians and engineers at Škoda Auto are working with version 3 of the MDF format, but slow adoption of version 4 is expected in the near future.
...
Another related tool from the same company is CANape, which focuses on the measurement and calibration of ECU parameters in real time. During measurement, engineers can adjust parameters while recording signals, with communication commonly handled via protocols such as XCP @asam_canape.
