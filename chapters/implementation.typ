...
A development environment is the tools and software developers use to write, test, and debug code. A good development environment can significantly improve productivity and make the development process more efficient. For this project, the development environment consisted of an #abbr("IDE", "Integrated Development Environment") and various tools for version control, testing, and documentation.

The primary IDE of choice for this project was PyCharm, a popular Python IDE developed by JetBrains. PyCharm offers a wide range of features that enhance the development experience, including intelligent code completion, code navigation, and integrated debugging tools. A heavier IDE like PyCharm was chosen over lighter code editors such as Visual Studio Code or Sublime Text because of its robust Python support and powerful features that help manage a larger codebase more effectively.

For the environment itself, the Nix package manager was used to create a reproducible development environment. Nix allows developers to define their development environment declaratively, specifying the exact versions of tools and libraries needed for the project. This ensures that all developers working on the project have a consistent environment, which can help avoid issues related to dependency conflicts or differences in tool versions while keeping everything isolated from the system environment @how_nix_works. The specific configuration used for this project is shown in @code:nix-dev-env, which includes Python 3.11 and the necessary development packages. Direnv was also used to automatically load the Nix environment when entering the project directory, further streamlining the development workflow @direnv.
...
Finally, Git was used for version control, primarily for good practice and to maintain a history of changes, rather than for collaboration, as the project was developed solely by me. Git allows developers to track changes to their code, revert to previous versions if needed, and manage different branches of development. When working on a new feature or a large refactor, branching allows developers to isolate their changes from the main codebase until they are ready to be merged, which can help prevent conflicts, maintain a cleaner commit history, and enable developers to work on multiple features or bug fixes simultaneously without interference @git. For this project, a simple branching strategy was used, with a main branch for stable code and feature branches for new development. Regular commits were made to the feature branches, and once a feature was complete and tested, it was merged back into the main branch using the rebase strategy to maintain a clean commit history. The project's source code was hosted on a self-hosted GitLab instance, providing a private repository for easy access and management of the codebase.

== Project structure

The project is structured as a single Python package named `graph_insights` containing all the source code for the application, accompanied by the main entrypoint script `main.py`. Supporting files, such as UI design files, static assets, and documentation, are organized into separate directories within the project.
...
This modular structure allows better code organization and makes it easier to maintain and extend the application in the future. Each module contains related functions and classes that encapsulate specific functionality, following the principles of separation of concerns and single responsibility. This organization also facilitates testing, as each module can be tested independently, and it allows for easier collaboration if other developers were to join the project in the future. The context of the various subpackages is outlined in @tab:dir-structure, which provides a brief description of each directory's purpose within the `graph_insights` package.
...
When a user wants to add new data to the application, they can do so by selecting a file (or multiple files) from their file system using a file dialog. The application must then asynchronously load these files in the background while keeping the user interface responsive. It must also inform the user of the loading progress so they are not left wondering whether the application is frozen or if something went wrong. This is typically achieved by using a separate thread or process to load data while the main thread continues to run the user interface. The application can then use signals and slots (or other inter-thread communication mechanisms) to update the user interface with progress information and to notify the user when the loading process is complete.

I have chosen a multithreaded pipeline for data ingestion. There are several distinct, ordered steps to perform when loading a data file. Keeping all the logic for these steps in a single function would make it very difficult to read and maintain. It also makes it easier to unit-test the individual steps of the loading process, as they can be tested in isolation. The pipeline approach also allows for better error handling, as errors can be caught and handled at the appropriate stage of the loading process, rather than having to catch all errors in a single function. The complete list of stages in the data loading pipeline is shown in table @tab:data-loading-pipeline.
...
When an Excel file is run through the pipeline, the `pandas` package is used to load it into a `DataFrame` object, which is a powerful data structure for handling tabular data and is the primary data structure used in the application. The Excel files we are working with have a specific format: the first row contains metadata, while the actual data starts in the second row. As per the example in @misc:metadata-string, the metadata contains the exact date and time when the record button was pressed, the internal identifier of the particular vehicle, and an arbitrary number of space-separated strings.
...
After metadata extraction, the pipeline filters out potential duplicates and unnamed columns, as these are not expected in the data and are likely to cause issues later. The next stage is to convert all columns to numeric types, as this is the expected format for the data and enables more efficient storage and processing. Any columns that cannot be converted to a numeric type are dropped from the `DataFrame`, as they are not useful for our purposes. The logic behind this stage is depicted in @diagram:convert-dtypes.
...
The next stage is to reindex the `DataFrame` from sequential integers to timestamps, using the `#Time` column as the source. This allows us to easily perform time-based operations on the data, such as resampling and rolling-window calculations. After reindexing, the time channel is dropped from the `DataFrame`, as it is no longer needed. The next stage is to derive new channels from existing ones using the rules defined in `processing/derived_channels.py`. Most commonly, these include power calculation from voltage and current channels. Finally, the column names are stripped of whitespace and normalized to a consistent case (lowercase) to avoid issues with inconsistent naming conventions in the source data#footnote("Change in column names is a frequent occurrence when switching between different versions of data recording software.").

== Designing the user interface

The user interface was to be simple, intuitive, and efficient for the target users, engineers, and technicians at Škoda Auto. The application needed to provide a clear, organized way to visualize and analyze tabular data exported from ODIS, with a focus on usability and accessibility for users with limited programming experience.

There are two ways to design a user interface in Qt: using code or using a visual design tool, preferably the official Qt Designer. For very simple applications, setting up all the widgets and layouts in code can be straightforward, quicker, and easier than wiring up a design file. However, for more complex applications with multiple windows, dialogs, and a variety of widgets, using a visual design tool is a no-brainer, as it allows developers to quickly and easily create and modify the user interface without writing much code. Qt Designer is bundled with PyQt6, so there is no need to install it separately. Recall @code:nix-dev-env, where I set up an alias for the bundled Qt Designer, which allows me to type ```bash designer``` in the terminal to launch the Qt Designer application.

Each window or dialog in the application was designed as a separate .ui file using Qt Designer. A UI file is simply an XML file that describes the layout and properties of the widgets in a particular window or dialog. Because of its text-based nature, it can be easily version-controlled and merged with Git, a significant advantage over binary design files used by some other frameworks. Loading a UI file in Python is straightforward, as shown in @code:load-ui. The ```uic.loadUi``` function takes the path to the .ui file and the instance of the widget to load it into, and it automatically creates the necessary widgets and layouts as defined in the design file.
...
The main window of the application is also the most important, as it serves as the central hub for all the application's functionality and is the first thing users see when they launch it. It must provide the following key features:
...
Before diving into the specifics of the visualization system, a rendering technology for the visualizations had to be chosen. There are several options for rendering visualizations in Python, each with its own advantages and disadvantages. A comparison of the most popular options available in the Python ecosystem is shown in @tab:graphing-library-comparison.
...
The table above is not an exhaustive comparison of all the visualization technologies available in Python, but it covers some of the most popular and widely used options. All of these are solid choices that provide all the necessary features for our use case, so the decision ultimately came down to personal preference and familiarity with the library. I drafted possible UI designs for both Matplotlib and Plotly, and the results of the informal user testing sessions with potential users leaned towards *Matplotlib*, as it provided a more traditional, familiar visualization experience that was preferred by the target users. It also helped that I had a few years of experience with Matplotlib, primarily through university assignments and personal projects.

With the chosen rendering technology, the next step was to design a flexible, extensible system for creating and managing visualizations within the application. The main goal was to create a system that would allow users to easily create and customize their visualizations, while also providing a solid foundation for future development and expansion of the application's visualization capabilities.

=== System architecture

The rendering system is quite complex, as it needs to handle a wide range of use cases and requirements. For that reason, I chose to break it into multiple parts (along with the diagrams) for this thesis. A diagram of the system as a whole is shown in Attachment 6, providing a high-level overview of all the components and their interactions.

At the heart of the system is the `Renderer` class. It works by instantiating a `Renderer` object with a certain specification, and then calling the `render()` method to generate the visualization. The `Renderer` class is designed to be flexible and extensible, allowing different rendering backends (e.g., Matplotlib, Plotly) and layout strategies (e.g., grid, stacked) to be used. The architecture of the core and data components is depicted in @diagram:rendering-system-core.
...
A _mixin_ is by definition a class that provides methods to other classes through inheritance, but is not intended to be instantiated on its own @mixin. In the rendering system, mixins extend the functionality of a `RenderBackend` by exposing additional methods specific to the mixin and by having the backend implement the necessary logic to support them, primarily rendering.

During the development, user feedback was collected and contributed to key features being added to the application, such as the ability to display the difference between two curves on a plot - a delta curve. This feature was implemented as a mixin (`DeltaMixin`), which exposes a `calculate_delta()` method that takes two curves, performs the necessary calculations to compute the delta curve, and returns the delta curve. It also defines an abstract method `plot_delta()` that must be implemented by any backend that uses the mixin, which takes care of rendering the delta curve on the plot. This design allows for clear separation of concerns: the mixin handles the logic for calculating the delta curve, while the backend renders it. The architecture of the `DeltaMixin` is depicted in @diagram:delta-mixin.
...
In practice, when a user selects the option to display the delta curve on a plot, the application does a few preliminary checks, namely whether the current context #footnote("A user selection involves zero or more files and channels. Depending on the layout, plots may contain a different number of curves. A delta curve is only calculated between exactly two curves.") and backend support it and if so, it calls the `calculate_delta()` method with the appropriate curves. The backend then performs the necessary calculations and calls the `plot_delta()` method to render the delta curve on the plot. Implementing the `plot_delta()` method also adds a baseline at y=0. It assigns the right y-axis to the delta curve, enabling better visualization and interpretation relative to the original curves. A concrete example of a plot with the delta curve rendered is shown in @img:delta-plot.
...
A plugin can be registered to any of these hooks, and it will be called at the appropriate time during rendering. The architecture of the plugin system is depicted in @diagram:plugins.
...
The application, with only the visualization features implemented, already provided significant value to users, enabling them to easily visualize and explore their data in ways not possible before. However, the ability to automatically extract insights and information from the data is what really takes the workflow to the next level. The goal is the following: as a user adds and removes files from the application over its lifetime, the application should automatically analyze the data in the background and extract insights, which are then presented to the user in a clear, organized way. This takes the time-consuming initial screening of the data off the users' shoulders and allows them to focus on the actual analysis and interpretation of the data, which is where their expertise lies.

I have split the process of analysis into two categories: *individual* and *comparative* #footnote("The term comparative in this context denotes a strictly pairwise framework, wherein analysis is conducted exclusively between two distinct files at a time."). This means that files are analyzed both individually and in comparison, targeting different use cases and enabling a more comprehensive analysis of the data.

Because the threaded pipeline pattern described and used in @section:data-ingestion worked so well for data loading, I decided to use a similar approach for the analysis features as well. Whenever a new file is added to the application, it passes through the processing pipeline. Once it is loaded and processed, the `on_processing_pipeline_batch_finished()` callback is triggered, which submits the file to the `IndividualAnalysisPipeline` for individual analysis. As a result, `submit_file_pairs_for_analysis()` is also called, which generates the file combinations and submits them into the `FilePairAnalysisPipeline` for comparative analysis. Both pipelines run in separate threads and execute their own sets of stages in order, similar to the data loading pipeline. The analysis results are then stored in a cache for quick access and emitted as signals to update the user interface with the new insights.

=== Channel data correlation <section:correlation>

In some scenarios, it may be useful to check whether certain channels are roughly (or exactly) the same across multiple files, for example, from a test drive conducted on multiple vehicles driven simultaneously. Hypothetically, a technician may want to confirm that both vehicles' radiator shutters (or a blind-like alternative, like the AAB @rochling_aab, which is more common nowadays in Škoda vehicles) behaved the same during the test drive, which could be an indication of a potential issue with the shutters if they did not. This is where correlation analysis comes in handy. By calculating the correlation coefficient between the channel `01/roleta chladiče, skutečná poloha/---` #footnote("Internal channel names often contain a mix of Czech, English, and German terminology.") across the corresponding files, the application can provide a quick and easy way for the user to verify whether the shutters behaved correctly or not.

Several correlation coefficients can be used to measure the correlation between two channels, including Pearson's correlation coefficient, Spearman's rank correlation coefficient, and Kendall's tau coefficient. Each of these coefficients has its own advantages and disadvantages, and the choice of which one to use depends on the specific characteristics of the data and the analysis being performed. For this application, I chose the *Pearson's correlation *coefficient as it is a widely used measure of linear correlation between two variables and is appropriate for the type of data being analyzed.

As for implementation, this feature falls under the category of comparative analysis, as it involves comparing channels across multiple files. Whenever a new file is added to the application, a set of distinct file combinations is generated using the `itertools.combinations()` function from the Python standard library. If the set of currently loaded files is denoted by #"F", then the number of combinations grows according to the following formula:
...
The set of files is then submitted to the `FilePairAnalysisPipeline`. The pipeline maintains a cache of previously calculated correlation coefficients for each pair of files, so if a combination has already been analyzed, the cached result is returned immediately without recalculation. If the combination is new, the pipeline calculates the correlation coefficient for each channel pair between the two files and stores the results in the cache for future reference.

For visualization purposes, the results are presented to the user as a table containing the list of correlated channels across multiple files, showing the correlation coefficient (color coded from red to blue, with red indicating strong negative correlation and blue indicating strong positive correlation) and a button to quickly view the two channels in question on a plot beside each other for a more detailed comparison, as shown in @img:correlations. The table columns are sortable, allowing users to quickly find the most correlated channels across their files, which can be a useful starting point for further analysis and investigation. Additional visualization methods for the correlation results are discussed in @chapter:the-future.
...
The results are again presented to the user in a tabular format, showing the list of channels in a file that contain anomalies (and the number of anomalies), along with a button to quickly view the channel on a plot with the anomalous points highlighted for more detailed analysis. Because this method falls under the category of individual analysis, the results are shown on a per-file basis by utilizing Qt's tab widget, which allows for a clear and organized presentation of the results while also providing an easy way for users to navigate between different files and their corresponding analysis results, as shown in @img:std-thresholding.
...
As mentioned, a button is provided for each channel with anomalies, allowing users to quickly view the channel on a plot with the anomalous points highlighted. The results of the analysis pipeline are a list of indices corresponding to the anomalous data points in the channel. To make this data more interpretable and actionable for users, the application constructs intervals from data points that are directly adjacent to each other, as these are likely to be part of the same anomaly, and highlights these intervals on the plot using a shaded area. A concrete example of such a plot is shown in @img:std-thresholding-plot, where the anomalous intervals are highlighted in red.
...
The statistical methods described in the previous section are a good starting point for anomaly detection and can reliably identify certain types of anomalies in the data. However, their entire data context consists only of the files loaded into the application, so any inconsistencies found are relative to the other data in the application, not necessarily absolute. For example, it cannot reveal illogical values in a channel that are not necessarily outliers in the context of the other loaded files. This is where #abbr("ML", "Machine Learning") can lend a helping hand. We keep pretty much all the data from previous test drives on a shared drive, so we have a large amount of historical data that can be used to train ML models to identify anomalies based on patterns and relationships that may not be immediately apparent through statistical methods alone. By leveraging ML techniques, the application can provide a more robust, comprehensive anomaly detection system that helps users identify potential issues and areas of interest in their data more effectively.

ML algorithms can be broadly categorized into supervised and unsupervised learning methods. Supervised learning methods require labeled data for training, which means that the anomalies in the historical data would need to be identified and labeled by experts, which can be a time-consuming and labor-intensive process. Unsupervised learning methods, on the other hand, do not require labeled data and can identify anomalies based on patterns and relationships within the data. Given that we want to dump a bunch of historical data into the model and have it train itself without manual labeling, *unsupervised* learning methods are the way to go for our use case. A couple of popular unsupervised ML algorithms for anomaly detection are compared in @tab:anomaly-detection-algorithms.
...
I decided to go with the Isolation Forest algorithm, as it is a powerful and widely used method for anomaly detection that is particularly effective for high-dimensional data, which is the case with our datasets. It works by randomly partitioning the data and isolating anomalies with fewer splits, allowing it to identify anomalies effectively even in complex datasets. The Isolation Forest algorithm is implemented in the `scikit-learn` library, making it ridiculously easy to use and integrate into the application.

I implemented the model training and validation in a separate script that runs independently of the main application, as the training process can be quite time-consuming and resource-intensive, and to avoid convoluting the main application codebase with ML-specific logic. The script takes a location on disk where the individual files are stored and a combination of channels that defines the model's dimensionality, and trains the model on data from these channels across all the files.

I used `scikit-learn` `Pipeline` class to create a pipeline that includes data preprocessing steps (e.g., scaling, dimensionality reduction) and the Isolation Forest algorithm itself. The trained model is then saved to disk using `joblib`, which users point the application at in the settings, and the application takes care of loading it and using it for anomaly detection (the whole pipeline is extracted from the `.joblib`, so the implementation on the application side stays very lean). This also allows training multiple models on different channel combinations or subsets of the data (e.g., winter vs. summer data), and easily switching between them in the application settings without modifying the codebase.
