...
Many factors influenced the choice of Python for this project (as summarized in table @tab:language-comparison), but the most important were my familiarity with the language and its ecosystem, as well as its suitability for rapid prototyping and development. The ease of development and maintainability were also important considerations, as the application is intended for engineers and technicians who may not have extensive programming experience. Python's simplicity and readability make it an excellent choice for this purpose, enabling developers to understand and modify the codebase as needed quickly.
...
Software frameworks provide a structured way to build applications by offering pre-built components and tools that handle common tasks, such as user interface design, event handling, and data management. For desktop applications, there are several popular Python frameworks, including PyQt, Tkinter, and Kivy.

Python is often used to build tools with simple graphical user interfaces, using lightweight frameworks such as Pygame or the aforementioned Tkinter. Frameworks like these are easy to learn and can be sufficient for basic applications. However, they may not provide the level of polish and user experience expected in a professional setting. For a more modern, responsive user interface, frameworks such as PyQt or Kivy are often preferred. PyQt is a set of Python bindings for the Qt application framework, widely used for developing cross-platform applications with a native look and feel. Kivy, on the other hand, is an open-source Python library for developing multitouch applications, which can also be used for desktop applications. Both frameworks have their own strengths and weaknesses, so the choice came down to the author's personal preference.

PyQt has a slightly steeper learning curve and can be more complex to set up. However, it offers a more traditional desktop application experience with a wide range of widgets and tools for building complex user interfaces. One particularly powerful feature of PyQt is its ```python QSettings``` system, which provides a convenient platform-independant way to store and retrieve application settings across sessions. For example, on Windows, settings are typically stored in the registry, while on Linux, they are stored in configuration files. By using QSettings' simple API (as shown in @code:qsettings-example), developers can abstract away these platform-specific details and ensure that user preferences and application state are preserved seamlessly across different operating systems.
...
The Qt framework (and consequently the PyQt library) has more than 25 years of development and a large community, making it well-documented and offering a wide range of resources for learning and troubleshooting. It has a robust layout system that enables developers to create complex, responsive user interfaces that adapt to different screen sizes and resolutions. Additionally, PyQt provides a wide range of widgets and tools for building modern desktop applications, which will be touched upon in the next section @qt_software. All of these factors contributed to the decision to use PyQt for this project, as it provides a powerful and flexible framework for building a professional-quality desktop application with a modern user interface.

== User workflow

The concept behind the application's main purpose is not complicated: it takes in data, the user decides exactly what and how they want to display it, and the application generates a visualization based on those choices. The main workflow should therefore be very simple and intuitive to actually perform its intended purpose - to save time for engineers and technicians. The general workflow is depicted in @img:user-workflow.
